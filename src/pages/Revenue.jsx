...
import { monthly, fmt, fmtN } from '../utils/data';
...
function Revenue() {
  ...
  const barConfig = {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: monthly.map((_, i) => i < 12 ? 'rgba(8,145,178,0.75)' : 'rgba(115,212,242,0.85)'),
        borderRadius: 4
      }]
    },
    options: {
  ...
  const compareConfig = {
    ...
    data: {
      ...
      datasets: [
        {
          label: '2025',
          data: monthly.slice(0, 7).map(d => d.rev),
          borderColor: '#0891B2',
        ...
        {
          label: '2026',
          data: monthly.slice(12, 19).map(d => d.rev),
          borderColor: '#73D4F2',
          backgroundColor: 'rgba(115,212,242,0.08)',
          fill: true,
          tension: 0.4,
          borderWidth: 2.5,
          pointRadius: 4,
    ...
    options: {
      responsive: true,
      maintainAspectRatio: false,
      plugins: { legend: { display: true, labels: { boxWidth: 10, padding: 10, color: '#334E5A' } }, tooltip: { callbacks: { label: ctx => ' ' + fmt(ctx.raw) } } },
      scales: {
        x: { grid: { color: 'rgba(184,233,248,0.7)' } },
        y: { grid: { color: 'rgba(184,233,248,0.7)' }, ticks: { callback: v => fmt(v) } }
      }
  ...
  const ordersConfig = {
    ...
    data: {
      labels: monthly.map(d => d.m),
      datasets: [{ data: monthly.map(d => d.orders), backgroundColor: 'rgba(3,105,161,0.7)', borderRadius: 3 }]
    },
    options: {
      responsive: true,
      maintainAspectRatio: false,
  ...
  return (
    <div className="page">
      <div className="kpi-grid">
        <KpiCard color="blue" label="2025 Full Year" value={fmt(fy25.rev)} change={`${fmtN(fy25.orders)} transactions`} />
        <KpiCard color="teal" label="2026 YTD (Jan–Jul)" value={fmt(ytd26.rev)} change={`${fmtN(ytd26.orders)} transactions`} />
        <KpiCard color="green" label="Best Month" value={best.m} valueClass="sm" change={`${fmt(best.rev)} revenue`} />
        <KpiCard color="amber" label="Avg Monthly Rev" value={fmt(avg)} change="19-month average" />
      </div>
