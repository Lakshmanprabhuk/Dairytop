...
function SalesReps() {
  ...
  const top3Share = useMemo(() => {
    const total = salesreps.reduce((s, r) => s + r.rev, 0);
    const top3Total = top3.reduce((s, r) => s + r.rev, 0);
    return ((top3Total / total) * 100).toFixed(0);
  }, [top3]);

  const barConfig = {
    type: 'bar',
    data: {
      labels: salesreps.map(d => d.n),
      datasets: [{ data: salesreps.map(d => d.rev), backgroundColor: PALETTE, borderRadius: 4 }]
    },
    options: {
      indexAxis: 'y',
      responsive: true,
  ...
  const donutConfig = {
    ...
    data: {
      ...
      datasets: [{ data: salesreps.map(d => d.rev), backgroundColor: PALETTE, borderWidth: 0 }]
    },
    options: {
      responsive: true,
      maintainAspectRatio: false,
      cutout: '62%',
      plugins: {
        legend: { position: 'bottom', labels: { boxWidth: 8, padding: 8, color: '#334E5A', font: { size: 9 } } },
  ...
  const tableRows = useMemo(() =>
    salesreps.map((d, i) => [i + 1, d.n, fmt(d.rev), fmtN(d.orders), fmt(Math.round(d.rev / d.orders))]),
    []
  );
  ...
  <Table headers={['#', 'Rep', 'Revenue', 'Orders', 'Avg Order']} rows={tableRows} />
</div>
    </div>
  );
}
